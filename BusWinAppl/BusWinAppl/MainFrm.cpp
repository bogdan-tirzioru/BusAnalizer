...
void CMainFrame::OnConnectUsb32772()
{
	...
	nrComPort = 21;
	const wchar_t * wc = mySettings.m_strListBox.GetString();
	swscanf_s(wc, L"Com %d", &nrComPort);
	switch (returnCode) 
	{
		case IDOK:
		...
		case IDCANCEL:
			//do something
			AfxMessageBox(_T("Cancel Connection!"), MB_OK | MB_ICONSTOP);
			break;
...
UINT  OneShoutRead(LPVOID Param)
{
	...
	const int nPortToUse = nrComPort;
...
#pragma warning(suppress: 26477)
	...
	#pragma warning(suppress: 26496)
			...
			try
			{
				port2.Read(buf.data(), 2048, overlapped);
				if (!buf.empty())
				{
					if (buf[0] != 0)
					{
